...
// VERIFICA se existe algum usuário logado na sessão.
export const getCurentUser = async () => {
    try {
        const currentAccount = await account.get(); // <-- get informaçãoes da conta que está logada.

        if (!currentAccount) throw Error;

        // PEGA informações da sessão para unir com as informações do
